...
import club.flame.flameutils.utils.MojangUtils;
...
public class MaintenanceCommand extends Command {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (args[0].equalsIgnoreCase("on")) {
            ...
            FlameUtils.getInstance().whitelist(true);
            return;
        }

        if (args[0].equalsIgnoreCase("off")) {
            sender.sendMessage(CC.translate("&cThe server is no longer whitelisted."));
            FlameUtils.getInstance().whitelist(false);
            return;
        }
        ...
        if (args[0].equalsIgnoreCase("list")) {
            ...
            String error = null;
            for (String uuid : FlameUtils.getInstance().getBungeeHandler().getWhitelists()) {
                try {
        ...
        try {
            if (args.length >= 2) {
                if (args[0].equalsIgnoreCase("add")) {
                    String uuid2 = args[1];
                    FlameUtils.getInstance().getBungeeHandler().setWhitelisted(Objects.requireNonNull(MojangUtils.fetchUUID(uuid2)).toString(), true);
                    sender.sendMessage(CC.translate("&aYou successfully added " + uuid2 + " to the whitelist!"));
                    return;
                }
                if (args[0].equalsIgnoreCase("remove")) {
                    String uuid2 = args[1];
                    FlameUtils.getInstance().getBungeeHandler().setWhitelisted(Objects.requireNonNull(MojangUtils.fetchUUID(uuid2)).toString(), false);
